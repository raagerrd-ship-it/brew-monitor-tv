...
- **Trickle-adjust (past-target 1%-step)**: När cooling & `avgError < -0.05°C` (eller heating & `avgError > +0.05°C`) och PID:s beräknade duty avviker ≥0.5% från `holdLockDuty` i "säker" riktning → ta EN 1%-step mot PID:s värde, sätt `holdLockDuty = ny nivå`, refresha `holdLockUntil` med nytt 15m-fönster, återställ `holdLockBaseline = ssotFiltered`. Ger mjuk 6→5→4→3-nedgång (ett steg per 15 min = 3 PID-cykler) utan risk för studs upp på burst-brus. Symmetriskt vid heating. Constraint `hold-lock-trickle(±1%→X%)`.
