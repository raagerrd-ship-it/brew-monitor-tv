-- Drop the old overloaded function with text parameters
DROP FUNCTION IF EXISTS public.get_temp_history_sampled(text, text, text, integer);

-- Recreate with explicit text parameters (matching how the client calls it)
...
-- Drop the timestamptz overload if it exists
DROP FUNCTION IF EXISTS public.get_temp_history_sampled(text, timestamp with time zone, timestamp with time zone, integer);