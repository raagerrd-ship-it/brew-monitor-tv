...
AS $function$
  ...
  SELECT
    ...
    COALESCE(MAX(CASE WHEN cooling_enabled THEN duty_pct ELSE 0 END) / 100.0, 0)::NUMERIC AS cooling_ratio,
    COALESCE(MAX(CASE WHEN NOT cooling_enabled THEN duty_pct ELSE 0 END) / 100.0, 0)::NUMERIC AS heating_ratio,
