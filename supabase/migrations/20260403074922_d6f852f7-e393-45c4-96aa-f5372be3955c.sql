...
-- Add actual_temp column (idempotent)
ALTER TABLE public.temp_controller_history
ADD COLUMN IF NOT EXISTS actual_temp numeric NULL;

-- Must drop first since return type changes
DROP FUNCTION IF EXISTS public.get_temp_history_sampled(text, text, text, integer);

CREATE FUNCTION public.get_temp_history_sampled(p_controller_id text, p_start_time text, p_end_time text, p_sample_interval_minutes integer DEFAULT 15)
 RETURNS TABLE(recorded_at timestamp with time zone, current_temp numeric, target_temp numeric, cooling_enabled boolean, profile_target_temp numeric, cooling_ratio numeric, actual_temp numeric)
 LANGUAGE sql
 STABLE
AS $function$
  WITH bucketed AS (
  ...
  SELECT
    ...
    COALESCE(MAX(duty_pct) / 100.0, 0)::NUMERIC AS cooling_ratio,
    (ARRAY_AGG(actual_temp ORDER BY recorded_at DESC))[1]::NUMERIC AS actual_temp
  FROM bucketed
  GROUP BY bucket
  ORDER BY bucket;
$function$;
