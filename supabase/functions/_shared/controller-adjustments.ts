...
import { round1, TempController, setControllerTargetTemp, loadPillCompSettings, calculateCompensatedTarget, RaptUpdateBatch, setCoolerHysteresis, setHeatingHysteresis, setHeatingEnabled, setCoolingEnabled } from './temp-utils.ts'
...
// ─── Smart Relay (Processor) ─────────────────────────────────
// Toggles heating/cooling relays based on direction and applies
// adaptive hysteresis tightening when off-target too long.

interface SmartRelaySettings {
  enabled: boolean
  coolingOnlyBelow: number
  heatingOnlyAbove: number
  minHysteresis: number
  tightenAfterMinutes: number
}

async function loadSmartRelaySettings(supabase: ReturnType<typeof createClient>): Promise<SmartRelaySettings> {
  const { data } = await supabase
    .from('auto_cooling_settings')
    .select('smart_relay_enabled, smart_relay_cooling_only_below, smart_relay_heating_only_above, smart_relay_min_hysteresis, smart_relay_tighten_after_minutes')
    .limit(1)
    .maybeSingle()
  if (!data) return { enabled: false, coolingOnlyBelow: 15, heatingOnlyAbove: 20, minHysteresis: 0.3, tightenAfterMinutes: 30 }
  return {
    enabled: (data as any).smart_relay_enabled ?? false,
    coolingOnlyBelow: parseFloat(String((data as any).smart_relay_cooling_only_below ?? 15)),
    heatingOnlyAbove: parseFloat(String((data as any).smart_relay_heating_only_above ?? 20)),
    minHysteresis: parseFloat(String((data as any).smart_relay_min_hysteresis ?? 0.3)),
    tightenAfterMinutes: parseInt(String((data as any).smart_relay_tighten_after_minutes ?? 30)),
  }
}
...
async function runSmartRelay(ctx: ControllerAdjustmentContext): Promise<AdjustmentResult[]> {
  ...
  if (!settings.enabled) {
    log('SMART_RELAY', 'info', 'Smart Relay disabled')
    return []
  }

  log('SMART_RELAY', 'info', `Smart Relay active (cool<${settings.coolingOnlyBelow}°C, heat>${settings.heatingOnlyAbove}°C, min_hyst=${settings.minHysteresis}°C, tighten=${settings.tightenAfterMinutes}min)`)

  for (const fc of followedControllersFullData) {
    // Skip glycol cooler — this is for tank controllers only
    if ((fc as any).is_glycol_cooler) continue
    if (!fc.heating_enabled && !fc.cooling_enabled) continue
    ...
    let wantCooling: boolean
    let wantHeating: boolean

    if (!holdZone) {
      // Direction-based: cooling or heating
      wantCooling = diff < 0 // target below actual → need cooling
      wantHeating = diff > 0 // target above actual → need heating
    } else {
      // Hold zone — use temperature bands
      if (actualTarget < settings.coolingOnlyBelow) {
        wantCooling = true
        wantHeating = false
      } else if (actualTarget > settings.heatingOnlyAbove) {
        wantCooling = false
        wantHeating = true
      } else {
        wantCooling = true
    ...
    if (!isSmartActive) {
      await supabase.from('rapt_temp_controllers').update({
        smart_relay_active: true,
        pre_smart_heating_enabled: fc.heating_enabled,
        pre_smart_cooling_enabled: fc.cooling_enabled,
        pre_smart_heating_hysteresis: fc.heating_hysteresis,
        pre_smart_cooling_hysteresis: fc.cooling_hysteresis,
        updated_at: new Date().toISOString(),
      }).eq('controller_id', fc.controller_id)
      log('SMART_RELAY', 'info', `${fc.name}: Saved original relay state (heat=${fc.heating_enabled}, cool=${fc.cooling_enabled}, h_hyst=${fc.heating_hysteresis}, c_hyst=${fc.cooling_hysteresis})`)
    }

    // Toggle relays if needed
    const currentHeating = fc.heating_enabled ?? false
    const currentCooling = fc.cooling_enabled ?? false

    if (wantHeating !== currentHeating) {
      const ok = await setHeatingEnabled(supabaseUrl, serviceRoleKey, fc.controller_id, wantHeating)
      if (ok) {
        log('SMART_RELAY', 'action', `${fc.name}: ${wantHeating ? 'Enabled' : 'Disabled'} heating (target=${actualTarget.toFixed(1)}°C, actual=${actualTemp.toFixed(1)}°C)`)
        ;(fc as any).heating_enabled = wantHeating
      }
    }

    if (wantCooling !== currentCooling) {
      const ok = await setCoolingEnabled(supabaseUrl, serviceRoleKey, fc.controller_id, wantCooling)
      if (ok) {
        log('SMART_RELAY', 'action', `${fc.name}: ${wantCooling ? 'Enabled' : 'Disabled'} cooling (target=${actualTarget.toFixed(1)}°C, actual=${actualTemp.toFixed(1)}°C)`)
        ;(fc as any).cooling_enabled = wantCooling
      }
    }

    // ── Adaptive Hysteresis ──
    const onTarget = Math.abs(actualTarget - actualTemp) < 0.5
    const offTargetSince: string | null = (fc as any).smart_relay_off_target_since

    if (onTarget) {
      // Restore original hysteresis if it was tightened
      if (offTargetSince) {
        const origCoolHyst = (fc as any).pre_smart_cooling_hysteresis
        const origHeatHyst = (fc as any).pre_smart_heating_hysteresis

        if (origCoolHyst != null && fc.cooling_hysteresis != null && Math.abs(fc.cooling_hysteresis - origCoolHyst) > 0.05) {
          await setCoolerHysteresis(supabaseUrl, serviceRoleKey, fc.controller_id, origCoolHyst)
          log('SMART_RELAY_RESTORE', 'action', `${fc.name}: Restored cooling hysteresis ${fc.cooling_hysteresis.toFixed(1)} → ${origCoolHyst.toFixed(1)}°C`)
        }
        if (origHeatHyst != null && fc.heating_hysteresis != null && Math.abs(fc.heating_hysteresis - origHeatHyst) > 0.05) {
          await setHeatingHysteresis(supabaseUrl, serviceRoleKey, fc.controller_id, origHeatHyst)
          log('SMART_RELAY_RESTORE', 'action', `${fc.name}: Restored heating hysteresis ${fc.heating_hysteresis.toFixed(1)} → ${origHeatHyst.toFixed(1)}°C`)
        }

        await supabase.from('rapt_temp_controllers').update({
          smart_relay_off_target_since: null,
          updated_at: new Date().toISOString(),
        }).eq('controller_id', fc.controller_id)
      }
    } else {
      // Off target — start or continue tightening
      if (!offTargetSince) {
        await supabase.from('rapt_temp_controllers').update({
          smart_relay_off_target_since: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        }).eq('controller_id', fc.controller_id)
      } else {
        const minutesOff = (Date.now() - new Date(offTargetSince).getTime()) / 60000
        if (minutesOff >= settings.tightenAfterMinutes) {
          // Tighten the active relay's hysteresis
          if (wantCooling && fc.cooling_hysteresis != null) {
            const newHyst = Math.max(settings.minHysteresis, round1(fc.cooling_hysteresis - 0.2) ?? settings.minHysteresis)
            if (Math.abs(newHyst - fc.cooling_hysteresis) >= 0.05) {
              await setCoolerHysteresis(supabaseUrl, serviceRoleKey, fc.controller_id, newHyst)
              log('SMART_RELAY_TIGHTEN', 'action', `${fc.name}: Tightened cooling hysteresis ${fc.cooling_hysteresis.toFixed(1)} → ${newHyst.toFixed(1)}°C (${Math.round(minutesOff)}min off-target)`)
              ;(fc as any).cooling_hysteresis = newHyst
            }
          }
          if (wantHeating && fc.heating_hysteresis != null) {
            const newHyst = Math.max(settings.minHysteresis, round1(fc.heating_hysteresis - 0.2) ?? settings.minHysteresis)
            if (Math.abs(newHyst - fc.heating_hysteresis) >= 0.05) {
              await setHeatingHysteresis(supabaseUrl, serviceRoleKey, fc.controller_id, newHyst)
              log('SMART_RELAY_TIGHTEN', 'action', `${fc.name}: Tightened heating hysteresis ${fc.heating_hysteresis.toFixed(1)} → ${newHyst.toFixed(1)}°C (${Math.round(minutesOff)}min off-target)`)
              ;(fc as any).heating_hysteresis = newHyst
            }
          }
          // Reset timer so tightening happens again next cycle
          await supabase.from('rapt_temp_controllers').update({
            smart_relay_off_target_since: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          }).eq('controller_id', fc.controller_id)
  ...
  // Smart Relay doesn't modify target_temp — returns no adjustments
  return []
}
