      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            {
              type: 'text',
              text: `Apply a Gaussian blur of ${blur}px. Analyze the image's overall brightness and adjust it so the final output has an average perceived brightness of approximately ${brightnessPercent}% of maximum. Bright images should be darkened significantly, while already dark images should be darkened less or not at all. The goal is a consistent output brightness regardless of the input. IMPORTANT: Apply a smooth dark gradient fade at the TOP of the image, spanning the top ~8% of the image height (approximately 60px at 720p). The gradient should go from about 60% black opacity at the very top edge to fully transparent. This ensures header text readability. Scale to 1280x720. Output as JPEG.`,
            },
            {
              type: 'image_url',
