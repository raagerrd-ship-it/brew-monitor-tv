  ...
  try {
    ...
    if (reqBody?.injected_controllers) {
      ...
      if (ids.length > 0) {
        const { data: ssotRows } = await supabase
          ...
          .select('controller_id, pill_temp, actual_temp, profile_target_temp, current_temp_updated_at, last_hw_push_at, pill_probe_offset, pill_probe_offset_baseline, hysteresis_kick_active, dual_sensor_enabled, preferred_sensor, pid_version')
    ...
    if (updateBatch.size > 0) {
      ...
      for (const [controllerId] of succeeded) {
        ...
        const pwmBurstDecision = isPwmSend ? decisionLog.find(d => (d.step === 'DUTY_BURST' || d.step === 'DUTY_FULL') && d.message?.includes(name)) : null;
