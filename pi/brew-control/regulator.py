...
class TankRegulator:
    ...
    # Telemetry buffers
    temp_buffer: deque = field(default_factory=lambda: deque(maxlen=300))  # 5 min @ 1Hz
...
class Regulator:
    ...
    def _tick_tank(self, tank: TankRegulator, glycol_temp: Optional[float], now: float):
        ...
        if temp is None or not math.isfinite(temp):
            ...
            self.relays.get_tank(tank.controller_id).all_off()
            tank.current_duty = 0
            return
        ...
        on_since = tank_relay.cool.on_since if tank.mode == "cooling" else tank_relay.heat.on_since
        ...
        if tank.pwm_thread and tank.pwm_thread.is_alive():
            # Previous PWM window still running — let it finish
            pass
        else:
    ...
    def _manage_compressor(self, glycol_temp: Optional[float], now: float):
        ...
        # Determine if any tank is actively cooling
        any_cooling = any(
            t.mode == "cooling" and t.current_duty > 0
            for t in self.tanks.values()
        )

        # Find the lowest cooling target among active tanks
        cooling_targets = [
        ...
        if glycol_temp is not None and startup_delay_passed:
            if any_cooling and glycol_temp > desired_glycol + config.GLYCOL_HYSTERESIS:
                ...
                should_run = True  # hysteresis keep-on
