              ...
              <Fragment key={controller.id}>
                {index > 0 && <div className="h-5 mx-2 w-px flex-shrink-0 rounded-full" style={{ background: 'hsl(0 0% 100% / 0.12)' }} />}
                 ...
                 {(() => {
                   ...
                   return (
                 <div className={`relative flex items-center justify-center rounded px-3 gap-2 ${isTvMode ? '' : 'cursor-pointer'}`} style={{ background: 'transparent', width: controller.is_glycol_cooler ? '100px' : '180px', paddingTop: '4px', paddingBottom: linkedPill ? '10px' : '4px' }}
