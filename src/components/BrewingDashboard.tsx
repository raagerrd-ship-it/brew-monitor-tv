...
const RaptControllerBar = memo(function RaptControllerBar({ controllers, pills, onControllerClick, isMobile, isTvMode = false }: RaptControllerBarProps) {
  return (
    <div className={isMobile ? "flex items-center justify-center w-full" : ""}>
      <div 
        className={`flex items-center rounded-lg ${isMobile ? 'gap-1 px-2 py-2' : 'gap-1.5 px-2 py-1.5'} overflow-x-auto scrollbar-hide`}
        style={{
          ...
          return (
            <div key={controller.id} className="flex items-center">
              ...
              >
                ...
                {linkedPill && (
                  <div 
                    className={`flex items-center gap-1 transition-opacity ${isPillStale ? 'opacity-40' : isMobile ? 'opacity-60' : ''}`}
                    title={!isMobile ? `${linkedPill.name}\nBatteri: ${linkedPill.battery_level}%${isPillStale ? '\n⚠️ Ingen uppdatering på >24h' : ''}` : undefined}
                  >
                    ...
                    <span 
                      className={`font-semibold tabular-nums whitespace-nowrap ${isMobile ? 'text-[10px]' : ''}`}
                      style={{ 
