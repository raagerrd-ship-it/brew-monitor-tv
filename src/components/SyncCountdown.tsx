...
export function SyncCountdown({ className = "" }: SyncCountdownProps) {
  ...
  useEffect(() => {
    // Load sync interval from database
    const loadSyncInterval = async () => {
      try {
        ...
        console.error('Error loading sync interval:', error);
      }
    };

    loadSyncInterval();

    // Subscribe to sync_settings changes to update the interval
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          if (payload.new && 'sync_interval' in payload.new) {
            setSyncInterval((payload.new as any).sync_interval);
          }
        }
      )
      .subscribe();
