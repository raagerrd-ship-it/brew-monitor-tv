...
function EntryRow({ entry, hideSync, hidePid, formatTime, recentCoolerAdjs }: {
  ...
  return (
    <Collapsible>
      <CollapsibleTrigger className="grid grid-cols-[auto_1fr_20px] items-center w-full py-2 px-3 bg-muted/30 rounded-lg hover:bg-muted/50 transition-colors gap-x-2 text-xs">
          ...
          <table className="w-full text-[10px]">
            ...
            <tbody>
              {pidStatusEntries.map((d, i) => {
                ...
                const comp = (det.compensation as number) ?? (delta != null ? delta / 2 : 0);
