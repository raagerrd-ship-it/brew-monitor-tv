...
/** Scale canvas to printer width (384px), keeping aspect ratio */
function scaleCanvasToPrinterWidth(source: HTMLCanvasElement): HTMLCanvasElement {
  const scale = PRINTER_WIDTH_PX / source.width;
  const newHeight = Math.round(source.height * scale);
  const out = document.createElement('canvas');
  out.width = PRINTER_WIDTH_PX;
  out.height = newHeight;
  const ctx = out.getContext('2d')!;
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'high';
  ctx.fillStyle = '#ffffff';
  ctx.fillRect(0, 0, PRINTER_WIDTH_PX, newHeight);
  ctx.drawImage(source, 0, 0, PRINTER_WIDTH_PX, newHeight);
  return out;
}
...
export async function printBitmap(
  ...
  const scaled = scaleCanvasToPrinterWidth(canvas);
